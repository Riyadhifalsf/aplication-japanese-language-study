    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 18),
          Text('Pilih permainan', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, index) {
                ...
                final selected = selectedGame == index;
                return SizedBox(
                  width: 150,
                  child: Card(
                    ...
                    child: InkWell(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Row(children: [Text(item.$2, style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: selected ? cs.primary : null)), const Spacer(), if (selected) Icon(Icons.check_circle_rounded, color: cs.primary, size: 18)]),
                            const Spacer(),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text('$stage', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900, color: unlocked ? cs.primary : cs.onSurfaceVariant)),
                  const SizedBox(height: 3),
                  Icon(unlocked ? Icons.play_circle_fill_rounded : Icons.lock_rounded, size: 18, color: unlocked ? cs.primary : cs.onSurfaceVariant),
                ],
              ),
            ),
          ),
...
class _StageGameScreenState extends State<StageGameScreen> {
  ...
  static const katakanaQuestions = [
    ('a', ['ア', 'オ', 'ウ', 'エ', 'カ']),
    ('ka', ['カ', 'キ', 'ク', 'ケ', 'コ']),
    ('shi', ['シ', 'サ', 'ス', 'セ', 'ソ']),
    ('ta', ['タ', 'チ', 'ツ', 'テ', 'ト']),
    ...
    showDialog<void>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(3, (i) => Icon(i < stars ? Icons.star_rounded : Icons.star_outline_rounded, size: 42, color: Theme.of(context).colorScheme.tertiary))),
        ...
        actions: [
          TextButton(onPressed: () { Navigator.pop(context); Navigator.pop(context); }, child: const Text('Kembali')),
          FilledButton(onPressed: () { Navigator.pop(context); setState(() { index = 0; score = 0; seconds = 15; answered = false; }); timer = Timer.periodic(const Duration(seconds: 1), (_) { if (!mounted || answered) return; if (seconds <= 1) { _answer(''); } else { setState(() => seconds--); } }); }, child: const Text('Main lagi')),
        ],
      ),
    );
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          ClipRRect(borderRadius: BorderRadius.circular(99), child: LinearProgressIndicator(value: (index + 1) / 10, minHeight: 9)),
          const SizedBox(height: 8),
          Row(children: [const Icon(Icons.timer_rounded, size: 18), const SizedBox(width: 5), Text('${seconds}s', style: TextStyle(fontWeight: FontWeight.w900, color: seconds <= 5 ? cs.error : cs.onSurface)), const Spacer(), Icon(Icons.star_rounded, color: cs.tertiary), const SizedBox(width: 4), Text('$score', style: const TextStyle(fontWeight: FontWeight.w900))]),
          const SizedBox(height: 18),
          Card(
            child: Padding(
          ...
          GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: 2.1),
            itemBuilder: (_, i) => FilledButton.tonal(
              onPressed: answered ? null : () => _answer(options[i]),
              child: Text(options[i], style: const TextStyle(fontSize: 23, fontWeight: FontWeight.w900)),
            ),
          ),
        ],
      ),
