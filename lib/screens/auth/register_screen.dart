...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  @override
  void dispose() {
    _name.dispose(); _email.dispose(); _password.dispose(); _confirmPassword.dispose(); super.dispose();
  }
  ...
  Future<void> _googleSignUp() async {
    ...
    try {
      ...
      final known = email.isNotEmpty && prefs.containsKey(_onboardingKey(email));
      final done = known ? (prefs.getBool(_onboardingKey(email)) ?? false) : false;
      Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => done ? const AppShell() : const OnboardingScreen()), (_) => false);
    } catch (_) {
      if (!mounted) return;
  ...
  Future<void> _register() async {
    ...
    if (_password.text != _confirmPassword.text) { setState(() => _error = 'Konfirmasi password tidak cocok.'); return; }
    setState(() { _busy = true; _error = ''; });
    final app = AppScope.of(context);
    final email = _email.text.trim().toLowerCase();
    final error = await app.registerUser(name: _name.text, email: email, password: _password.text);
    if (!mounted) return;
    if (error != null) { setState(() { _busy = false; _error = error; }); return; }
    if (!app.isAuthenticated) {
      final loginError = await app.loginWithEmail(email, _password.text);
      if (!mounted) return;
      if (loginError != null) { setState(() { _busy = false; _error = loginError; }); return; }
    }
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_onboardingKey(email), false);
    setState(() => _busy = false);
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 520), child: ListView(padding: const EdgeInsets.fromLTRB(24, 12, 24, 28), children: [
      Center(child: Image.asset('assets/branding/japanese_study_logo.png', width: 148, height: 148, fit: BoxFit.contain)),
      const SizedBox(height: 24),
      FilledButton.icon(onPressed: _busy ? null : _googleSignUp, icon: const GoogleGIcon(), label: const Text('Daftar dengan Google'), style: FilledButton.styleFrom(minimumSize: const Size.fromHeight(54), backgroundColor: Colors.white, foregroundColor: Colors.black87)),
      const SizedBox(height: 20),
      Row(children: [Expanded(child: Divider(color: Theme.of(context).colorScheme.outlineVariant)), const Padding(padding: EdgeInsets.symmetric(horizontal: 12), child: Text('atau daftar dengan email')), Expanded(child: Divider(color: Theme.of(context).colorScheme.outlineVariant))]),
